...
function formatBRL(value: number) {
  return parseFloat(value?.toString() || '0').toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
}

function getOffsetMonth(year: number, month: number, offset: number) {
...
export function Dashboard() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Load accounts and tasks for header cards
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-white dark:bg-earth-900 p-5 rounded-2xl shadow-sm border border-earth-200 dark:border-earth-800">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-earth-500">Tarefas Pendentes</p>
              <h3 className="text-2xl font-bold text-orange-600 mt-1">{loading ? '...' : pendingTasks}</h3>
            </div>
