...
class FinanceDataService:
    ...
    def import_data(user, payload: dict) -> dict:
        ...
        with db_transaction.atomic():
            # Mapa de nome → instância para contas e categorias
            ...
            for tx_data in payload.get('transactions', []):
                try:
                    ...
                    account = acc_map.get(acc_name) if acc_name else None
                    if not account:
                        errors.append(f"Conta '{acc_name}' não encontrada — transação ignorada.")
                        continue

                    to_acc_name = tx_data.get('to_account_name')
                    to_account = acc_map.get(to_acc_name) if to_acc_name else None
